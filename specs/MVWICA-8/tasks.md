...
- [ ] T013 [US1] Implement `TemperatureProducer`: Avro-encode `TemperatureReadingEvent` and publish to `health-metrics.temperature`; raise `KafkaUnavailableError` on broker failure — repo: `sapphire-event-ingestion-api` `src/kafka/temperature_producer.py`
...
- [ ] T022 [US2] Add `temperature_readings` hypertable creation to database initialisation script (if not already done in T002) with chunk interval 1 day and composite index `(user_id, measured_at DESC)` — repo: `sapphire-kafka-pipeline` `sql/001_temperature_schema.sql`
- [ ] T023 [US2] Add `temperature_daily_rollups` continuous aggregate and refresh policy to initialisation script (if not already done in T003) — repo: `sapphire-kafka-pipeline` `sql/002_temperature_rollups.sql`
